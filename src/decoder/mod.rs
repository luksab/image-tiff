...
impl<'a> DecodingBuffer<'a> {
    ...
    fn chunks_mut(self, chunk_size: usize) -> Box<dyn Iterator<Item = DecodingBuffer<'a>> + 'a> {
        match self {
            DecodingBuffer::U8(buf) => Box::new(buf.chunks_mut(chunk_size).map(DecodingBuffer::U8)),
            DecodingBuffer::I8(buf) => Box::new(buf.chunks_mut(chunk_size).map(DecodingBuffer::I8)),
            DecodingBuffer::U16(buf) => {
                Box::new(buf.chunks_mut(chunk_size).map(DecodingBuffer::U16))
            }
            DecodingBuffer::I16(buf) => {
                Box::new(buf.chunks_mut(chunk_size).map(DecodingBuffer::I16))
            }
            DecodingBuffer::U32(buf) => {
                Box::new(buf.chunks_mut(chunk_size).map(DecodingBuffer::U32))
            }
            DecodingBuffer::I32(buf) => {
                Box::new(buf.chunks_mut(chunk_size).map(DecodingBuffer::I32))
            }
            DecodingBuffer::U64(buf) => {
                Box::new(buf.chunks_mut(chunk_size).map(DecodingBuffer::U64))
            }
            DecodingBuffer::I64(buf) => {
                Box::new(buf.chunks_mut(chunk_size).map(DecodingBuffer::I64))
            }
            DecodingBuffer::F32(buf) => {
                Box::new(buf.chunks_mut(chunk_size).map(DecodingBuffer::F32))
            }
            DecodingBuffer::F64(buf) => {
                Box::new(buf.chunks_mut(chunk_size).map(DecodingBuffer::F64))
            }
        }
    }
}
